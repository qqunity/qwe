...
std::vector<unsigned int> Minimax::getAvailSpots(Matrix<char> mat) {
...
int Minimax::minimax(Matrix<char> mat, char symbol) {
    this->game.restart();
...
unsigned int Minimax::getBestMove(Matrix<char> mat) {
    std::vector<unsigned int> availSpots = this->getAvailSpots(mat);
    unsigned int move = 0;
