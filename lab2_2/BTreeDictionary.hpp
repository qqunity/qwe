#ifndef LAB_2_BTREEDICTIONARY_HPP
#define LAB_2_BTREEDICTIONARY_HPP
...
namespace dictionary {
    ...
    class BTreeDictionary: public IDictionary<K, V> {
    private:
        tree::BTree<KVElement<K, V>> *tree;
    public:
        BTreeDictionary(unsigned int t, const std::string &sequenceType, K defaultKey, V defaultValue);

        BTreeDictionary(const BTreeDictionary<K, V> &dict);
        ...
        friend std::ostream &operator<<(std::ostream &out, BTreeDictionary<K1, V1> &dict);
    };

    template<typename K, typename V>
    BTreeDictionary<K, V>::BTreeDictionary(unsigned int t, const std::string &sequenceType, K defaultKey, V defaultValue): IDictionary<K, V>(t, sequenceType, defaultKey, defaultValue) {
        this->tree = new tree::BTree<KVElement<K, V>>(t, sequenceType, this->defaultElement);
    }

    template<typename K, typename V>
    BTreeDictionary<K, V>::BTreeDictionary(const BTreeDictionary<K, V> &dict) {
        this->t = dict.t;
        ...
        this->size = 0;
        this->tree = new tree::BTree<KVElement<K, V>>(this->t, this->sequenceType, this->defaultElement);
    ...
    template<typename K, typename V>
    void BTreeDictionary<K, V>::insert(K key, V value) {
        ...
        if (this->tree->search(*element) == this->defaultElement) {
            ...
            throw exceptions::BTreeDictionaryException("Key already exists", __LINE__, __FILE__);
        }
    }

    template<typename K1, typename V1>
    std::ostream &operator<<(std::ostream &out, BTreeDictionary<K1, V1> &dict) {
        if (dict.size != 0) {
            out << "{";
            for (unsigned int i = 0; i < dict.size; ++i) {
    ...
    template<typename K, typename V>
    KVElement<K, V> &BTreeDictionary<K, V>::operator[](K key) {
    ...
    template<typename K, typename V>
    KVElement<K, V> &BTreeDictionary<K, V>::get(K key) {
        auto *element = new KVElement<K, V>(key, this->defaultElement.getValue());
        if (this->tree->search(*element) != this->defaultElement) {
            return this->tree->search(*element);
        } else {
            throw exceptions::BTreeDictionaryException("Key has no exists", __LINE__, __FILE__);
        }
    }

    template<typename K, typename V>
    bool BTreeDictionary<K, V>::contains(K key) {
        auto *element = new KVElement<K, V>(key, this->defaultElement.getValue());
        return this->tree->search(*element) != this->defaultElement;
    }

    template<typename K, typename V>
    void BTreeDictionary<K, V>::remove(K key) {
        auto *element = new KVElement<K, V>(key, this->defaultElement.getValue());
        if (this->tree->search(*element) != this->defaultElement) {
    ...
    template<typename K, typename V>
    sequence::SequenceIterator<V> BTreeDictionary<K, V>::values() {
    ...
    template<typename K, typename V>
    sequence::SequenceIterator<Pair<K, V>> BTreeDictionary<K, V>::items() {
    ...
    template<typename K, typename V>
    BTreeDictionary<K, V>::~BTreeDictionary() = default;
