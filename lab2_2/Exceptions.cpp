...
exceptions::GameException::GameException(const std::string& errMessage, int line, const std::string& file) {
    this->errorMessage = "-Error in line " + std::to_string(line) +
                         " in file" + file + "-\t-" +
                         " Message: " + errMessage + "-";
}

const char *exceptions::GameException::what() const noexcept {
    return this->errorMessage.c_str();
}
