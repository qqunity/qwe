...
#define COLLECTIONS_BINARYHEAP_H
    ...
    namespace binaryHeap {
        ...
        public:
            ...
            BinaryHeap(array::Array<T>, Fun);

            template<typename Fun>
            void siftDown(int, Fun);
            ...
            friend std::ostream &operator<<(std::ostream &, BinaryHeap<T1> &);
        };

        template<typename T>
        template<typename Fun>
        BinaryHeap<T>::BinaryHeap(array::Array<T> Arr, Fun ratioFunction) {
            this->data = Arr;
        ...
        void BinaryHeap<T>::siftDown(int node, Fun ratioFunction) {
            if (2 * node + 2 < this->data.length()) {
                if (ratioFunction(this->data[node], this->data[node * 2 + 1]) ||
                    ratioFunction(this->data[node], this->data[node * 2 + 2])) {
